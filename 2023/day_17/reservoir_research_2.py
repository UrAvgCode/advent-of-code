# --- Day 17: Reservoir Research --- Part Two ---
...
if __name__ == '__main__':
    ...
    while crucible_queue:
        ...
        if x == len(heat_map[y]) - 1 and y == len(heat_map) - 1 and moves_count >= 4:
            min_heat_loss = min(min_heat_loss, heat_loss)
            break

        next_x, next_y = x + x_dir, y + y_dir
        if moves_count < 10 and 0 <= next_x < len(heat_map[0]) and 0 <= next_y < len(heat_map):
